...
class SubjectDependencyRelationWindow(QDialog):
    def __init__(self, widgets, book_1, book_2):
        super(SubjectDependencyRelationWindow, self).__init__()
        ...
        self._relation_1 = self._magyarlanc_controller.get_dependency_relation_counts(self._book_1.writer, self._book_1.filename, "SUBJ")
        self._relation_2 = self._magyarlanc_controller.get_dependency_relation_counts(self._book_2.writer, self._book_2.filename, "SUBJ")
        ...
        self.create_subject_dependency_relation_bar_plot(self._relation_figure_1, self._relation_canvas_1, self._relation_1[0])
        self.create_subject_dependency_relation_bar_plot(self._relation_figure_2, self._relation_canvas_2, self._relation_2[0])
        ...
        self.create_subject_pos_bar_plot(self._pos_figure_1, self._pos_canvas_1, self._relation_1[2])
        self.create_subject_pos_bar_plot(self._pos_figure_2, self._pos_canvas_2, self._relation_2[2])
        ...
        self.create_subject_avg_bar_plot(self._avg_figure, self._avg_canvas, self._relation_1[3], self._relation_2[3])

    def set_labels(self):
        title_font = QFont("Arial", 14)
        title_font.setBold(True)

        self.title_label.setText("A mondatok alanyainak előfordulása")
        self.title_label.setFont(title_font)
        self.title_label.setWordWrap(True)

        self.relation_label_1.setText(f"{self._book_1.title} című műben, a mondatok alanyai közül legtöbbszőr előforduló 10 szótő.")
        ...
        self.relation_label_2.setText(f"{self._book_2.title} című műben, a mondatok alanyai közül legtöbbszőr előforduló 10 szótő.")
        ...
        self.avg_label.setText("A két mű mondataiban átlagosan hányszor szerepel az alany.")
    ...
    def create_subject_dependency_relation_bar_plot(self, figure, canvas, root_dict):
        figure.clear()
    ...
    def create_subject_pos_bar_plot(self, figure, canvas, pos_dict):
        figure.clear()
    ...
    def create_subject_avg_bar_plot(self, figure, canvas, avg_root_1, avg_root_2):
        figure.clear()

        averages = [float(round(avg_root_1, 2)), float(round(avg_root_2, 2))]

        ax = figure.add_subplot(111)
        bars = ax.bar([self._book_1.title, self._book_2.title], averages, color="peru",
                               label="Leghosszabb mondat", width=0.2)
